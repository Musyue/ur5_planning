...
class VisonControl():
    ...
    def callback_cross_line(self, msg):
        if len(self.cross_uv) >= 300:
            ...
            self.cross_uv.append(msg.uvinfo)
    # def caculate_avg_cross_line_uv(self,uvlist):
    #     if len(uvlist)==100:
    #         for i in uvlist:
    def get_list_uv(self,uvlist):
        reslut_u=[]
        reslut_v = []
        if len(uvlist)!=0:
            for i in uvlist:
...
def main():
    ...
    crossline_sub = rospy.Subscriber('/intersection_uv', uv, p0.callback_cross_line)
    ...
    while not rospy.is_shutdown():
        ...
        if len(p0.cross_uv)>0:
            ...
            print "wait cross line detecting uv publish---------"
            time.sleep(0.5)
            print "len(p0.cross_uv)",len(p0.cross_uv)
